...
### Initialize ABM
phil_abm = CHANCE_C.Simulator(phil_bg, phil_cbsa_base_pop, f_dict, f_matrix, CHANCE_C.model_step!; no_of_years = no_of_years, no_hhs_per_agent = no_hhs_per_agent,
...
function Agent_Location(agent::CHANCE_C.Queue, model::ABM; levee = false, f_e = 0.0, bg_sample_size = 10, house_choice_mode = "simple_anova_utility",
    ...
    if agent.type == :Relocating
        ...
        bg_sample = DataFrame(hh_id = hh_ids, bg_id = bg_ids, 
        GEOID = bg_GEOID, cat = bg_cat, bg_utility = bg_utilities)
        return bg_sample
        """
...
phil_abm.tick += 1
#clear utilities df
empty!(phil_abm.hh_utilities_df)
#create new agents
CHANCE_C.AgentMigration(phil_abm; phil_abm.agent_creation...)
